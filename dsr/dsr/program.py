...
class Program(object):
    ...
    def assemble_sequence(self):
        ''' Create full sequence from the master sequence and the generated mutations
            from thr RL controller. This is only needed for when no neighborhood
            is used by the RL agent. '''
        # subsequence generated by the RL agent
        short_seq = self.traversal
        new_sequence = list()
        # get master sequence and fill the blanks with RL's proposed mutations
        short_seq_pos = 0
        for i, aa in enumerate(self.extra_info['master_seq']):
            # fill the blanks
            if i in self.extra_info['allowed_mutations']:
                new_sequence.append(short_seq[short_seq_pos])
                short_seq_pos += 1
            else:
                new_sequence.append(Token(None, aa, arity=1, complexity=1))
        return new_sequence
    ...
    def __repr__(self):
        ...
        if self.task.task_type == 'binding':
            return ','.join([repr(t) for t in self.assemble_sequence()])
